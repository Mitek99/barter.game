...
- **Signature verification**: Every inbound request is verified against its `pubkey` before any handler runs. The `to` field must match the recipient bank's pubkey. The signed-REST UI API (`/:bank/ui/*`) authenticates with an `X-Barter-Auth` header: canonical authdoc `{pubkey, method, path, id, ts, body_sha256}` + signature, ±120 s timestamp skew, single-use `id`. The signed `path` is router-visible — under a path-prefixed mount (`BANK_MOUNT_PREFIX`, e.g. a gateway serving `/bank/{name}` and stripping the prefix) clients sign the `sign_base` from `GET /:bank/ui/config`, never the browser-visible path.
...
- **Double-spend gate**: an atomic KV check-and-set on the active-hold key enforces at most one active hold per account per external deal, and the hold acquisition itself re-checks debit solvency inside the same atomic — net of other deals' holds, floored at `max(0, debit_account_limit)` for non-issuer holder-authorized debits (`packages/bank-core/src/db.ts` `acquireHold`). Conflicts never error outward: the advance engine quietly issues no hold signatures that pass and re-attempts on later events (a stalled deal is eventually rejected by the bank's stall timeout — see "Banks self-advance" below).
- **Sum invariant**: on every settle, balances across all accounts for a Voucher must sum to zero (or the agreed limit).
- **Web analytics (opt-in)**: `BANK_POSTHOG_KEY` / `BANK_<NAME>_POSTHOG_KEY` (SAM `PosthogKey` parameter) makes `serveSpa` inject `window.__POSTHOG_KEY__` into the SPA; without it the web client never loads posthog-js and makes no analytics calls. Identify by pubkey only, never seed/keystore/password; credential screens carry `ph-no-capture` (see `apps/web/README.md`).
...
- **Banks self-advance**: clients submit docs (`submit_docs`); the coordinator creates records (`create_records`) and clears them (`submit_mandate`); from there each bank advances its own records `created → approved → held → settled` event-driven — re-evaluating on `submit_mandate` and `notify_signatures` (NOT `submit_docs`), with no cron, serialized per deal by an in-process mutex. Settle commits balance + hold release + the settle signature in ONE atomic batch (`db.ts` `applySettlement`), so a crash mid-settle cannot double-apply a delta. A mandated deal with no visible progress past the **stall timeout** is rejected and its holds released (lazy check inside the advance pass; `Bank.stallTimeoutMs`, default 1h, host override `BANK_STALL_TIMEOUT_MS`); the timeout-reject never fires once any settle signature for the deal has been observed. Signatures travel bank-to-bank directly (Address registry), with `get_record_signatures` + `notify_signatures` as the manual relay floor. `reject` is bank-issued only and cascades per deal.
