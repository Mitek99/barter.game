...
For the base doc shell, `Signature`, `Address`, canonicalization, and the JSON-RPC envelope, see [`base.md`](./base.md). For the settlement signature actions and the `seen` handshake, see [`settlement.md`](./settlement.md). For the RPC method definitions, see [`bank-rpc.md`](./bank-rpc.md). The `Post` doc type is defined in [`post-feed.md`](./post-feed.md). For the human narrative and trust/settlement models, see [`README.md`](./README.md).
...
`debit_account_limit` is a holder-set floor and may be negative, but a negative floor extends credit only under the issuer's own authorization (§3.2). For a transfer authorized by a non-issuer holder Order, the effective floor is `max(0, debit_account_limit)`: the bank MUST NOT process a debit that would drive a non-issuer account below zero, whatever floor the Order sets.
...
10. The resulting balance of the debit account does not fall below the effective floor: `O.debit_account_limit` if set, clamped to `max(0, O.debit_account_limit)` when the holder is not the voucher's issuer (§3.2).
...
How the Mandate's once-only acceptance and completeness checks protect against replayed and incomplete transactions is covered in [`settlement.md`](./settlement.md) §2.
...
```
   ...
   └────┬─────┘  for its Order is received. The bank now knows the deal's FULL
        ...
        ▼
   ┌──────────┐   HOLD is a cross-bank handshake carried by `seen` (`settlement.md` §3–4):
